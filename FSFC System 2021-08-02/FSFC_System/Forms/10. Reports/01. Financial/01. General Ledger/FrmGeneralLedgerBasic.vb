...
Public Class FrmGeneralLedgerBasic
    ...
    Private Sub FrmGeneralLedger_Basic_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        ...
        Catch ex As Exception
            TriggerBugReport("General Ledger Basic - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Public Sub LoadData()
        ...
        SQL &= "    `Source`,"
        SQL &= "    `Code` AS 'Mother Account Code',"
        SQL &= "    Title AS 'Mother Account',"
        ...
        SQL &= String.Format("    FROM account_chart LEFT JOIN (SELECT ID, EntryType, MotherCode AS 'AccountCode', Amount, IF(CVNumber = '',IF(CVNum = '',IF(ORNum = '',IF(JVNum = '','',JVNum),ORNum),CVNum),CVNumber) AS 'Source' FROM accounting_entry WHERE `status` = 'ACTIVE' AND PostStatus = 'POSTED' AND IF({2} = 1,TRUE,ORDate BETWEEN '{0}' AND '{1}') AND IF({3} = 1,Branch_ID IN ({7}),Branch_ID = '{4}') AND PaidFor NOT IN ('RPPD-A','RPPD-W','Penalty-W') AND IF({5} > 0, BankID = '{5}',TRUE) AND IF('{6}' = '0',TRUE,BusinessCode = '{6}') AND IF({8} = True,TRUE,IF('{9}' = 0,Book(BankID) = '{10}',BankID = '{9}'))) A ON A.AccountCode = account_chart.`Code` WHERE `status` IN ('ACTIVE','DEACTIVATE') AND Main_ID = 0 GROUP BY A.ID ORDER BY `Code`;", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), Format(If(cbxDisplay.SelectedIndex = 5, dtpFrom.Value, dtpTo.Value), "yyyy-MM-dd"), cbxAll.Checked, cbxAllB.Checked, cbxBranch.SelectedValue, DefaultBankID, ValidateComboBox(cbxBusinessCenter), Multiple_ID, cbxAllBank.Checked, ValidateComboBox(cbxBank), ValidateComboBox(cbxBook))
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Cursor = Cursors.WaitCursor
        Dim Report As New RptGeneralLedgerBasic
        With Report
            ...
            Logs("General Ledger", "Print", "[SENSITIVE] Print General Ledger - Basic List", "", "", "", "")
            .ShowPreview()
        End With
        Cursor = Cursors.Default
    End Sub
