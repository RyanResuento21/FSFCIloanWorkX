...
Public Class FrmGeneralLedgerPeriodicV2
    ...
    Private Sub FrmGeneralLedger_PeriodicV2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        ...
        Catch ex As Exception
            TriggerBugReport("General Ledger Periodic - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Public Sub LoadData()
        ...
        SQL &= String.Format("    (IFNULL(SUM(CASE WHEN IF(AccountFormula(account_chart.type_ID,account_chart.ContraAccount) = 1,EntryType = 'DEBIT',EntryType = 'CREDIT') AND AccountCode = `Code` AND IF({1} = 1,TRUE,ORDate < '{0}') THEN Amount END),0) - IFNULL(SUM(CASE WHEN IF(AccountFormula(account_chart.type_ID,account_chart.ContraAccount) = 1,EntryType = 'CREDIT',EntryType = 'DEBIT') AND AccountCode = `Code` AND IF({1} = 1,TRUE,ORDate < '{0}') THEN Amount END),0)) AS 'Beginning Balance',", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), cbxAll.Checked)
        SQL &= String.Format("    IFNULL(SUM(CASE WHEN EntryType = 'DEBIT' AND AccountCode = `Code` AND IF({2} = 1,TRUE,ORDate BETWEEN '{0}' AND '{1}') THEN Amount END),0) AS 'Debit',", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), Format(If(cbxDisplay.SelectedIndex = 5, dtpFrom.Value, dtpTo.Value), "yyyy-MM-dd"), cbxAll.Checked)
        SQL &= String.Format("    IFNULL(SUM(CASE WHEN EntryType = 'CREDIT' AND AccountCode = `Code` AND IF({2} = 1,TRUE,ORDate BETWEEN '{0}' AND '{1}') THEN Amount END),0) AS 'Credit',", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), Format(If(cbxDisplay.SelectedIndex = 5, dtpFrom.Value, dtpTo.Value), "yyyy-MM-dd"), cbxAll.Checked)
        SQL &= String.Format("    (IFNULL(SUM(CASE WHEN IF(AccountFormula(account_chart.type_ID,account_chart.ContraAccount) = 1,EntryType = 'DEBIT',EntryType = 'CREDIT') AND AccountCode = `Code` AND IF({2} = 1,TRUE,ORDate BETWEEN '{0}' AND '{1}') THEN Amount END),0) - IFNULL(SUM(CASE WHEN IF(AccountFormula(account_chart.type_ID,account_chart.ContraAccount) = 1,EntryType = 'CREDIT',EntryType = 'DEBIT') AND AccountCode = `Code` AND IF({2} = 1,TRUE,ORDate BETWEEN '{0}' AND '{1}') THEN Amount END),0)) AS 'Ending Balance', `Status`", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), Format(If(cbxDisplay.SelectedIndex = 5, dtpFrom.Value, dtpTo.Value), "yyyy-MM-dd"), cbxAll.Checked)
        SQL &= String.Format("    FROM account_chart LEFT JOIN (SELECT ID, EntryType, AccountCode, DepartmentCode, Amount, ORDate, IF(CVNumber = '',IF(CVNum = '',IF(ORNum = '',IF(JVNum = '','',JVNum),ORNum),CVNum),CVNumber) AS 'Source' FROM accounting_entry WHERE `status` = 'ACTIVE' AND PostStatus = 'POSTED' AND IF({3} = 1,Branch_ID IN ({7}),Branch_ID = '{4}') AND PaidFor NOT IN ('RPPD-A','RPPD-W','Penalty-W') AND IF({5} > 0, BankID = '{5}',TRUE) AND IF('{6}' = '0',TRUE,BusinessCode = '{6}') AND IF({8} = True,TRUE,IF('{9}' = 0,Book(BankID) = '{10}',BankID = '{9}'))) A ON A.AccountCode = account_chart.`Code` WHERE `status` IN ('ACTIVE','DEACTIVATE') AND Main_ID > 0 GROUP BY A.ID ORDER BY `Code`;", Format(If(cbxDisplay.SelectedIndex = 5, CDate("1/1/1753"), dtpFrom.Value), "yyyy-MM-dd"), Format(If(cbxDisplay.SelectedIndex = 5, dtpFrom.Value, dtpTo.Value), "yyyy-MM-dd"), cbxAll.Checked, cbxAllB.Checked, cbxBranch.SelectedValue, DefaultBankID, ValidateComboBox(cbxBusinessCenter), Multiple_ID, cbxAllBank.Checked, ValidateComboBox(cbxBank), ValidateComboBox(cbxBook))
        ...
        For x As Integer = 0 To DT.Rows.Count - 1
            ...
            If DT(x)("Formula") = 1 Then
                Beginning += CDec(DT(x)("Beginning Balance"))
                Ending += CDec(DT(x)("Ending Balance"))
            Else
                Beginning -= CDec(DT(x)("Beginning Balance"))
                Ending -= CDec(DT(x)("Ending Balance"))
            End If
        Next
        DT.Rows.Add(0, "T O T A L", "", "", "", "", "", 0, 0, 0, Beginning, Debit_Total, Credit_Total, Ending)
        ...
        If DT_ColumnSettings.Rows.Count > 0 Then
            ...
            GridColumn9.VisibleIndex = DT_ColumnSettings(0)("Column8V")
            GridColumn5.VisibleIndex = DT_ColumnSettings(0)("Column9V")
            GridColumn6.VisibleIndex = DT_ColumnSettings(0)("Column10V")
            GridColumn7.VisibleIndex = DT_ColumnSettings(0)("Column11V")
            ...
            GridColumn9.Width = DT_ColumnSettings(0)("Column8W")
            GridColumn5.Width = DT_ColumnSettings(0)("Column9W")
            GridColumn6.Width = DT_ColumnSettings(0)("Column10W")
            GridColumn7.Width = DT_ColumnSettings(0)("Column11W")
        Else
    ...
    Private Sub GridView1_Keydown(sender As Object, e As KeyEventArgs) Handles GridView1.KeyDown
        ...
        If e.Control And e.KeyCode = Keys.S Then
            If MsgBoxYes("Are you sure you want to save this Table Display") = MsgBoxResult.Yes Then
                ...
                SQL &= String.Format(" Column8V = '{0}', ", GridColumn9.VisibleIndex)
                SQL &= String.Format(" Column9V = '{0}', ", GridColumn5.VisibleIndex)
                SQL &= String.Format(" Column10V = '{0}', ", GridColumn6.VisibleIndex)
                SQL &= String.Format(" Column11V = '{0}', ", GridColumn7.VisibleIndex)
                ...
                SQL &= String.Format(" Column8W = '{0}', ", GridColumn9.Width)
                SQL &= String.Format(" Column9W = '{0}', ", GridColumn5.Width)
                SQL &= String.Format(" Column10W = '{0}', ", GridColumn6.Width)
                SQL &= String.Format(" Column11W = '{0}' ", GridColumn7.Width)
                If GridControl1.Tag = 1 Then
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        Cursor = Cursors.WaitCursor
        Dim Report As New RptGeneralLedgerPeriodicV2
        With Report
            ...
            Logs("General Ledger", "Print", "[SENSITIVE] Print General Ledger - Detailed Periodic List", "", "", "", "")
            .ShowPreview()
        End With
        Cursor = Cursors.Default
    End Sub
