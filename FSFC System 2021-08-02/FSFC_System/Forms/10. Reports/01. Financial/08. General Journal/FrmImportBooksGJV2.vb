...
Public Class FrmImportBooksGJV2
    ...
    Private Sub FrmImportBooksGJ_v2_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Temp_DT = DataSource(String.Format("SELECT 0 AS 'No', '' AS 'Reference No','' AS 'Payee','' AS 'Document Date','' AS 'Posting Date',0 AS 'BankID','' AS 'Bank','' AS 'Explanation','' AS 'Account Code','' AS 'Department Code','' AS 'Account','' AS 'Business Center','' AS 'Department',0.00 AS 'Debit',0.00 AS 'Credit','' AS 'PaidFor','' AS 'Remarks','' AS 'BusinessCode','' AS 'Type_ID','' AS 'MotherCode','' AS 'Prepared By','' AS 'Checked By','' AS 'Approved By', False AS 'RequiredRemarks', 0 AS 'PreparedID', 0 AS 'CheckedID', 0 AS 'ApprovedID' LIMIT 0"))
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        Try
            ...
            GetComboBoxFontSettings({cbxBank, cbxAccount, cbxDepartment, cbxBusinessCenter, cbxPreparedBy, cbxCheckedBy, cbxApprovedBy})
        ...
        Catch ex As Exception
            TriggerBugReport("Import Books GJ - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Private Sub BtnImport_Click(sender As Object, e As EventArgs) Handles btnImport.Click
        ...
        If OFD.ShowDialog = DialogResult.OK Then
            Using con As New OleDb.OleDbConnection()
                ...
                Catch ex As Exception
                    Cursor = Cursors.Default
                    TriggerBugReport("Import Books JV - Import", ex.Message.ToString)
                Finally
    ...
    Private Sub BtnVerify_Click(sender As Object, e As EventArgs) Handles btnVerify.Click
        ...
        If MsgBoxYes("Are you sure you want to Validate this data?") = MsgBoxResult.Yes Then
            ...
            For x As Integer = 0 To GridView2.RowCount - 1
                ...
                'WARNING
                If DataObject(String.Format("SELECT IFNULL(ReferenceNumber,'') FROM journal_voucher WHERE ReferenceNumber = '{0}' AND DocumentDate = '{1}' AND Branch_ID = '{2}' AND `status` = 'ACTIVE'", GridView2.GetRowCellValue(x, "Reference No").ToString, Format(DateValue(GridView2.GetRowCellValue(x, "Document Date")), "yyyy-MM-dd"), Branch_ID)) = GridView2.GetRowCellValue(x, "Reference No").ToString Then
            ...
            If MsgBoxYes(mSave) = MsgBoxResult.Yes Then
                ...
                For x As Integer = 0 To GridView2.RowCount - 1
                    ...
                    If DocumentNumber <> PrevDocumentNumber Then
                        ...
                        SQL &= String.Format(" OTAC_Check = '{0}', ", Format(Date.Now, "MMddHHmm"))
                        SQL &= String.Format(" OTAC_Approve = '{0}', ", Format(Date.Now, "MMddHHmm"))
                        SQL &= String.Format(" voucher_status = '{0}', ", If(GridView2.GetRowCellDisplayText(x, "CheckedID").ToString = "" And GridView2.GetRowCellDisplayText(x, "ApprovedID").ToString = "", "PENDING", If(GridView2.GetRowCellDisplayText(x, "CheckedID").ToString <> "" And GridView2.GetRowCellDisplayText(x, "ApprovedID").ToString = "", "CHECKED", "APPROVED")))
                    ...
                    SQL &= String.Format(" Debit = '{0}', ", CDbl(If(GridView2.GetRowCellValue(x, "Debit").ToString = "", 0, GridView2.GetRowCellValue(x, "Debit"))))
                    SQL &= String.Format(" Credit = '{0}', ", CDbl(If(GridView2.GetRowCellValue(x, "Credit").ToString = "", 0, GridView2.GetRowCellValue(x, "Credit"))))
                    SQL &= String.Format(" Remarks = '{0}';", GridView2.GetRowCellValue(x, "Remarks").ToString.InsertQuote)

                    'Accounting Entries
                    SQL &= "INSERT INTO accounting_entry SET"
                    SQL &= String.Format(" JVNum = '{0}', ", DocumentNumber)
                    SQL &= String.Format(" ORDate = '{0}', ", Format(DateValue(GridView2.GetRowCellDisplayText(x, "Posting Date")), "yyyy-MM-dd"))
                    SQL &= String.Format(" ReferenceN = '{0}', ", DocumentNumber)
    ...
    Private Sub BtnExport_Click(sender As Object, e As EventArgs) Handles btnExport.Click
        If MsgBoxYes("Are you sure you want to export this to Excel Format?") = MsgBoxResult.Yes Then
            Cursor = Cursors.WaitCursor
            Dim FName As String = Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) & "\" & "General Journal Format" & "-" & Format(Date.Now, "yyyy-MM-dd_HHmmss") & ".xlsx"
            Dim Report As New RptExportGeneralJournal
            With Report
