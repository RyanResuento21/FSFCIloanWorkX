...
Public Class FrmAccountsPayable
    ...
    Private Sub FrmAccountsPayable_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim DT_Status As DataTable = DataSource("SELECT 'For Checking' AS 'Status' UNION SELECT 'For Approval' UNION SELECT 'For CV / ACR' UNION SELECT 'Partially Paid' UNION SELECT 'Fully Paid' UNION SELECT 'Non Payable' UNION SELECT 'Cancelled'")
        With cbxStatus
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        Try
            ...
            GetLabelFontSettings({LabelX2, LabelX14, LabelX1, LabelX18, LabelX10, LabelX15, LabelX6, LabelX9, LabelX5, LabelX12, LabelX34, LabelX40, LabelX42, LabelX39, LabelX41})

            GetLabelWithBackgroundFontSettings({LabelX7, LabelX21, LabelX3, LabelX19, LabelX4, LabelX8, LabelX35})

            GetCheckBoxFontSettings({cbxCOD, cbx30D, cbx60D, cbx90D, cbxOthersT, cbxAll})

            GetButtonFontSettings({btnAdd_Account, btnRemove_Account, btnAdd, btnSave, btnRefresh, btnCancel, btnDelete, btnModify, btnDisapprove, btnPrint, btnAttach, btnCheck, btnApprove})

            GetComboBoxFontSettings({cbxPayee, cbxBank, cbxAccount, cbxDepartment, cbxBusinessCenter, cbxPreparedBy, cbxDisplay})

            GetDateTimeInputFontSettings({dtpDocument, dtpPostingDate, dtpDelivery, dtpFrom, dtpTo})
        ...
        Catch ex As Exception
            TriggerBugReport("Accounts Pyable - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Private Sub LblTitle_Click(sender As Object, e As EventArgs) Handles lblTitle.Click
        OpenFormHistory("Accounts Payable", lblTitle.Text)
    End Sub
    ...
    Public Sub LoadData()
        ...
        SQL &= "    Payee,"
        SQL &= "    Payee_Type,"
        SQL &= "    (SELECT CONCAT('Bank ', `Code`, ' [', IF(AccountNumber = '',CheckingAccount,AccountNumber), '] ', (SELECT B.short_name FROM bank_setup B WHERE B.ID = BankID)) AS 'Bank' FROM branch_bank WHERE ID = accounts_payable.BankID) AS 'Bank', BankID, "
        ...
        SQL &= "    BRANCH(branch_id) AS 'Branch', User_EmplID, Branch_ID, IF(NotAP = 1,'NON PAYABLE',IF(`Status` IN ('DELETED','CANCELLED','DISAPPROVED'),`status`,IF(JVNumber != '','REVERSED',IF(AP_Status='PENDING','FOR CHECKING',IF(AP_Status='CHECKED','FOR APPROVAL',IF(AP_Status='APPROVED','FOR CV / ACR',AP_Status)))))) AS 'AP_Status', Employee(CheckedID) AS 'Checked By', Employee(ApprovedID) AS 'Approved By', OTAC_Check, OTAC_Approve, Attach, JVNumber, JVNumberV2, NotAP"
        SQL &= "  FROM accounts_payable WHERE"
        ...
        Next
        If Cancelled Then
            If ForChecking = False And ForApproval = False And ForCV = False And PartiallyPaid = False And FullyPaid = False Then
                If Cancelled Then
                    SQL &= " (`status` IN ('CANCELLED','DELETED','DISAPPROVED') OR IF(NotAP = 1,'NOT PAYABLE',AP_Status) = 'DISAPPROVED') "
                End If
            Else
    ...
    Private Sub GetDocumentNumber()
        If btnSave.Text = "&Save" Then
            txtDocumentNumber.Text = DataObject(String.Format("SELECT CONCAT('AP-', '{1}', '{2}', '-', LPAD(COUNT(ID) + 1,4,'0')) FROM accounts_payable WHERE branch_id = '{0}' AND YEAR(DocumentDate) = YEAR('{3}');", Branch_ID, Branch_Code, Format(dtpDocument.Value, "yy"), Format(dtpDocument.Value, "yyyy-MM-dd")))
        End If
    End Sub
...
#Region "Keydown"
    ...
    Private Sub DtpDelivery_KeyDown(sender As Object, e As KeyEventArgs) Handles dtpDelivery.KeyDown
        If e.KeyCode = Keys.Enter Then
            rExplanation.Focus()
        End If
    End Sub
...
#End Region
    ...
    Private Sub GridView2_CellValueChanged(sender As Object, e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GridView2.CellValueChanged
        ...
        ElseIf e.Column.FieldName = "Credit" Then
            ...
            For x As Integer = 0 To DT_Account.Rows.Count - 1
                ...
                If GridView2.GetRowCellValue(x, "Account").ToString.Contains("Accounts Payable") Then
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
        ...
        If btnSave.Text = "&Save" Then
            If MsgBoxYes(mSave) = MsgBoxResult.Yes Then
                ...
                GetDocumentNumber()
                If DocumentNumberExist("accounts_payable", txtDocumentNumber.Text) Then
                    Exit Sub
                End If
                Dim SQL As String = "INSERT INTO accounts_payable SET"
                ...
                SQL &= String.Format(" Delivery_Date = '{0}', ", FormatDatePicker(dtpDelivery))
                ...
                For x As Integer = 0 To GridView2.RowCount - 1
                    ...
                    Else
                        SQL &= "INSERT INTO ap_details SET"
                        SQL &= String.Format(" DocumentNumber = '{0}', ", txtDocumentNumber.Text)
                        ...
                        ElseIf GridView2.GetRowCellValue(x, "Account").ToString.Contains("Due to") Then
                            Dim BranchTagged As Integer = DataObject(String.Format("SELECT BranchTagged FROM account_chart WHERE `Code` = '{0}' AND `status` = 'ACTIVE';", GridView2.GetRowCellValue(x, "Account Code")))
                            Dim APNumber As String = DataObject(String.Format("SELECT CONCAT('DT-', '{1}', '{2}', '-', LPAD(COUNT(ID) + 1,4,'0')) FROM due_to WHERE branch_id = '{0}' AND YEAR(DocumentDate) = YEAR('{3}');", Branch_ID, Branch_Code, Format(dtpDocument.Value, "yy"), Format(dtpDocument.Value, "yyyy-MM-dd")))
                ...
                For x As Integer = 0 To DT_Account.Rows.Count - 1
                    ...
                    Else
                        ...
                        SQL &= String.Format(" BusinessCode = '{0}', ", GridView2.GetRowCellValue(x, "BusinessCode"))
                        SQL &= String.Format(" DepartmentCode = '{0}', ", GridView2.GetRowCellValue(x, "Department Code"))
                        SQL &= String.Format(" PaidFor = '{0}', ", "Accounts Payable")
                        SQL &= " `Status` = 'PENDING',"
                ...
                Logs("Accounts Payable", "Save", String.Format("Add new Accounts Payable for {0} with Amount {1}.", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dAmount.Text), "", "", "", "")
                Clear(True)
                Cursor = Cursors.Default
        ...
        Else
            If MsgBoxYes(mUpdate) = MsgBoxResult.Yes Then
                Dim CancelledID As Integer = DataObject(String.Format("SELECT IFNULL(ID,0) FROM accounts_payable WHERE ID = {0} AND `status` IN ('CANCELLED','DISAPPROVED')", ID))
                ...
                Dim SQL As String = "UPDATE accounts_payable SET"
                SQL &= String.Format(" PayeeID = '{0}', ", ValidateComboBox(cbxPayee))
                SQL &= String.Format(" Payee = '{0}', ", cbxPayee.Text.InsertQuote)
                SQL &= String.Format(" Payee_Type = '{0}', ", If(cbxPayee.SelectedIndex = -1, "", drv("Type")))
                SQL &= String.Format(" Terms = '{0}', ", If(cbxCOD.Checked, 0, If(cbx30D.Checked, 30, If(cbx60D.Checked, 60, If(cbx90D.Checked, 90, If(cbxOthersT.Checked, iOthersD.Value, 0))))))
                ...
                SQL &= String.Format("UPDATE ap_details SET `status` = 'CANCELLED' WHERE DocumentNumber = '{0}' AND `status` = 'ACTIVE';", txtDocumentNumber.Text)
                ...
                Logs("Accounts Payable", "Update", Reason, Changes(), "", "", "")
                Clear(True)
                Cursor = Cursors.Default
    ...
    Private Function Changes()
        ...
        Try
            If cbxPayee.Text.InsertQuote = cbxPayee.Tag Then
            Else
            ...
            Else
                Change &= String.Format("Change Type from {0} to {1}. ", cbxCOD.Tag, If(cbxCOD.Checked, "C.O.D.", If(cbx30D.Checked, "30 Days", If(cbx60D.Checked, "60 Days", If(cbx90D.Checked, "90 Days", If(cbxOthersT.Checked, iOthersD.Value & " Days", ""))))))
            End If
            If dtpDelivery.Text = dtpDelivery.Tag Then
            Else
                Change &= String.Format("Change Delivery Date from {0} to {1}. ", dtpDelivery.Tag, dtpDelivery.Text)
            End If
            If dtpPostingDate.Text = dtpPostingDate.Tag Then
            Else
            ...
            End If
        Catch ex As Exception
            TriggerBugReport("Account Payable - Changes", ex.Message.ToString)
        End Try
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MsgBoxYes(mDelete) = MsgBoxResult.Yes Then
            ...
            Cursor = Cursors.WaitCursor
            Dim SQL As String = String.Format("UPDATE accounts_payable SET `status` = 'CANCELLED' WHERE ID = '{0}';", ID)
            ...
            DataObject(SQL)
            Logs("Accounts Payable", "Cancel", Reason, String.Format("Cancel Accounts Payable of {0} with Amount {1}.", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dAmount.Text), "", "", "")
            Clear(True)
            Cursor = Cursors.Default
    ...
    Private Sub Generate_AP(Show As Boolean, FName As String, CheckedBy As String, ApprovedBy As String)
        ...
        With Report
            .Name = String.Format("Accounts Payable of {0} - {1}", cbxPayee.Text, txtDocumentNumber.Text)
            ...
            .lblDelivery.Text = dtpDelivery.Text
            .lblReferenceNumber.Text = txtReferenceNumber.Text
            .cbxCOD.Checked = cbxCOD.Checked
            .cbx30D.Checked = cbx30D.Checked
            .cbx60D.Checked = cbx60D.Checked
            .cbx90D.Checked = cbx90D.Checked
            .cbxOthersT.Checked = cbxOthersT.Checked
            .iOthersD.Text = iOthersD.Text
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        If SuperTabControl1.SelectedTabIndex = 0 Then
            ...
            Logs("Accounts Payable", "Print", "[SENSITIVE] Print Accounts Payable " & txtDocumentNumber.Text, "", "", "", "")
        Else
            GridView1.OptionsPrint.UsePrintStyles = False
            StandardPrinting("ACCOUNTS PAYABLE LIST", GridControl1)
            Logs("Accounts Payable", "Print", "[SENSITIVE] Print Accounts Payable List", "", "", "", "")
        End If
        Cursor = Cursors.Default
    End Sub
    '***BUTTON CLICK
    ...
    Public Sub GridView1_DoubleClick(sender As Object, e As EventArgs) Handles GridView1.DoubleClick
        ...
        For x As Integer = 0 To DT_Account.Rows.Count - 1
            TotalDebit += CDbl(DT_Account(x)("Debit"))
            TotalCredit += CDbl(DT_Account(x)("Credit"))
            If GridView2.GetRowCellValue(x, "Account").Contains("Accounts Payable") Then
                Total_CIB += CDbl(DT_Account(x)("Credit"))
            End If
        Next
        dDebitT.Value = TotalDebit
        ...
        SuperTabControl1.SelectedTab = tabSetup
        If GridView1.GetFocusedRowCellValue("AP_Status") = "PENDING" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CHECKING" Then
            ...
            If User_Type = "ADMIN" Or Empl_ID = User_EmplID Or Empl_ID = GridView1.GetFocusedRowCellValue("PreparedID") Then
                ...
                btnCheck.Visible = True
            End If
        ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "CHECKED" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR APPROVAL" Then
            For x As Integer = 0 To DT_Signatory.Rows.Count - 1
            ...
            If User_Type = "ADMIN" Or Empl_ID = User_EmplID Or Empl_ID = GridView1.GetFocusedRowCellValue("PreparedID") Then
                btnApprove.Visible = True
            End If
        ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "APPROVED" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CV / ACR" Then
            btnCheck.Visible = False
            btnApprove.Visible = False
            btnModify.Enabled = False
        End If
    ...
    Private Sub CbxPayee_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbxPayee.SelectedIndexChanged
        ...
        Try
            ...
            Else
                ...
                If drv("Terms") = 0 Then
                    cbxCOD.Checked = True
                ElseIf drv("Terms") = 30 Then
                    cbx30D.Checked = True
                ElseIf drv("Terms") = 60 Then
                    cbx60D.Checked = True
                ElseIf drv("Terms") = 90 Then
                    cbx90D.Checked = True
                Else
                    cbxOthersT.Checked = True
                    iOthersD.Value = drv("Terms")
                End If
            End If
    ...
    Private Sub BtnAttach_Click(sender As Object, e As EventArgs) Handles btnAttach.Click
        ...
        With Attach
            .FolderName = "Accounts Payable-" & GridView1.GetFocusedRowCellValue("Document Number")
            .APNumber = GridView1.GetFocusedRowCellValue("Document Number")
    ...
    Private Sub BtnCheck_Click(sender As Object, e As EventArgs) Handles btnCheck.Click
        ...
        If (User_EmplID = Empl_ID Or cbxPreparedBy.SelectedValue = Empl_ID) And Signatory = False Then
            ...
            End With
        Else
            If MsgBoxYes("Are you sure you check this Accounts Payable?") = MsgBoxResult.Yes Then
                Cursor = Cursors.WaitCursor
                Code = GenerateOTAC()
                '*********** A U T O   N O T I F I C A T I O N *************************************************************************
                If Auto_Notification Then
                ...
                Logs("Accounts Payable", "Check", String.Format("Checked Accounts Payable of {0} with the total amount of {1}.", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
    ...
    Private Sub BtnApprove_Click(sender As Object, e As EventArgs) Handles btnApprove.Click
        ...
        If (User_EmplID = Empl_ID Or cbxPreparedBy.SelectedValue = Empl_ID) And Signatory = False Then
                ...
                If Result = DialogResult.OK Then
                    ...
                    With Provider
                        ...
                        If .ShowDialog = DialogResult.OK Then
                            Cursor = Cursors.WaitCursor
                            Dim SQL As String
                            SQL = String.Format("UPDATE accounting_entry SET ORDate = '{1}', `Status` = 'ACTIVE', PostStatus = 'POSTED' WHERE CVNumber = '{0}' AND `status` = 'PENDING';", txtDocumentNumber.Text, Format(dtpPostingDate.Value, "yyyy-MM-dd"))

                            DataObject(String.Format(SQL & "UPDATE accounts_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE ID = '{0}'; UPDATE accounts_receivable SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE due_from SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE due_to SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE loans_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}';", ID, .cbxProvider.SelectedValue, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                            Logs("Accounts Payable", "Approve", String.Format("Approved Accounts Payable of {0} with the total amount of {1}. [Via OTAC]", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
            ...
            End With
        Else
            If MsgBoxYes("Are you sure you want to approve this Accounts Payable?") = MsgBoxResult.Yes Then
                Cursor = Cursors.WaitCursor
                ...
                DataObject(String.Format(SQL & "UPDATE accounts_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE ID = '{0}'; UPDATE accounts_receivable SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE due_from SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE due_to SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE loans_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}';", ID, Empl_ID, Code, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                Logs("Accounts Payable", "Approve", String.Format("Approved Accounts Payable of {0} with the total amount of {1}.", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
    ...
    Private Sub BtnDisapprove_Click(sender As Object, e As EventArgs) Handles btnDisapprove.Click
        If MsgBoxYes("Are you sure you want to disapprove this Accounts Payable?") = MsgBoxResult.Yes Then
            Dim Reason As String 'Reason for change
            If FrmReason.ShowDialog = DialogResult.OK Then
                Reason = FrmReason.txtReason.Text
            Else
            ...
            Cursor = Cursors.WaitCursor
            Dim SQL As String = String.Format("UPDATE accounts_payable SET `status` = 'DISAPPROVED' WHERE ID = '{0}';", ID)
            ...
            DataObject(SQL)
            Logs("Accounts Payable", "Disapprove", Reason, String.Format("Disapprove Accounts Payable of {0} with Amount {1}.", cbxPayee.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "")
            Clear(True)
            Cursor = Cursors.Default
    ...
    Private Sub ICheckVoucher_Click(sender As Object, e As EventArgs) Handles iCheckVoucher.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR APPROVAL" Then
                MsgBox("Accounts Payable is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FULLY PAID" Then
                MsgBox("Accounts Payable is already FULLY PAID.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("NotAP") = 1 Then
                MsgBox("Accounts Payable is not for Check Voucher.", MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        With CV
            ...
            Else
                ...
                Exit Sub
            End If
            Logs("Accounts Payable", "Check Voucher", "Check Voucher", "", "", "", "")
            ...
            .AP_Account = DataObject(String.Format("SELECT AccountCode FROM ap_details WHERE DocumentNumber = '{0}' AND Credit > 0 AND `status` = 'ACTIVE' LIMIT 1;", GridView1.GetFocusedRowCellValue("Document Number")))
            ...
            If selectedRowHandles.Length > 1 Then
                ...
                For x As Integer = 0 To selectedRowHandles.Length - 1
                    ...
                    Else
                        ...
                        Exit Sub
                    End If
                    If Rows(x)("AP_Status") = "APPROVED" Or Rows(x)("AP_Status") = "FOR CV / ACR" Or Rows(x)("AP_Status") = "PARTIALLY PAID" Then
                    Else
                        MsgBox(String.Format("Selected Accounts Payable have a {0} status.", Rows(x)("AP_Status")), MsgBoxStyle.Information, "Info")
                        Exit Sub
                    End If
    ...
    Private Sub INotAP_Click(sender As Object, e As EventArgs) Handles iNotAP.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("JVNumberV2") = "" Then
                MsgBox("Accounts Payable is not auto generated.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR APPROVAL" Then
                MsgBox("Accounts Payable is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FULLY PAID" Or GridView1.GetFocusedRowCellValue("AP_Status") = "PARTIALLY PAID" Then
                MsgBox("Accounts Payable already have a transaction.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "CANCELLED" Or GridView1.GetFocusedRowCellValue("AP_Status") = "DELETED" Or GridView1.GetFocusedRowCellValue("AP_Status") = "DISAPPROVED" Then
                MsgBox(String.Format("Accounts Payable is already {0}.", GridView1.GetFocusedRowCellValue("AP_Status")), MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        If MsgBoxYes(String.Format("Are you sure you want to set this as {0}Accounts Payable?", If(GridView1.GetFocusedRowCellValue("NotAP") = 0, "Not ", ""))) = MsgBoxResult.Yes Then
            Dim Reason As String 'Reason for change
            If FrmReason.ShowDialog = DialogResult.OK Then
                Reason = FrmReason.txtReason.Text
            Else
            ...
            Dim SQL As String = String.Format("UPDATE accounts_payable SET NotAP = {1} WHERE ID = '{0}';", GridView1.GetFocusedRowCellValue("ID"), If(GridView1.GetFocusedRowCellValue("NotAP") = 0, 1, 0))
            DataObject(SQL)
            Logs("Accounts Payable", "Non AP", Reason, "", "", "", "")
            LoadData()
            MsgBox("Successfully Updated!", MsgBoxStyle.Information, "Info")
        End If
    End Sub
    ...
    Private Sub BtnRename_Click(sender As Object, e As EventArgs) Handles btnRename.Click
        ...
        With Rename
            ...
            If .ShowDialog = DialogResult.OK Then
                Dim SQL As String = String.Format("UPDATE accounts_payable SET Payee = '{1}' WHERE ID = '{0}';", GridView1.GetFocusedRowCellValue("ID"), .txtPayee.Text.InsertQuote)
                DataObject(SQL)
                Logs("Accounts Payable", "Change Payee", String.Format("Changed Payee for {0} from {1} to {2}", GridView1.GetFocusedRowCellValue("Payee").ToString.InsertQuote & " - " & GridView1.GetFocusedRowCellValue("Document Number"), GridView1.GetFocusedRowCellValue("Payee").ToString.InsertQuote, .txtPayee.Text.InsertQuote), "", "", "", "")
                LoadData()
    ...
    Private Sub IJV_Click(sender As Object, e As EventArgs) Handles iJV.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("JVNumber") <> "" Then
                MsgBox("Accounts Payable is already reversed. Please check your data.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AP_Status") = "FOR APPROVAL" Then
                MsgBox("Accounts Payable is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FOR CV / ACR" Or GridView1.GetFocusedRowCellValue("AP_Status") = "NON PAYABLE" Then
            Else
                MsgBox(String.Format("Accounts Payable is already {0}.", GridView1.GetFocusedRowCellValue("AP_Status")), MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        ...
        With JV
            ...
            Else
                ...
                Exit Sub
            End If
            Logs("Accounts Payable", "Journal Voucher", "Journal Voucher", "", "", "", "")
            .cbxLA.Enabled = False
            .cbxCV.Enabled = False
        ...
        If txtChecked.Text = "" Then
            ...
            For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                If DT_Signatory(x)("ButtonID") = btnCheck.Tag And DT_Signatory(x)("FormID") = Tag Then
                    ...
                    Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for CHECKING of Updated Accounts Payable of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayee.Text, dDebitT.Text, cbxPreparedBy.Text)
                    Msg &= "Thank you!"
                    '******* SEND SMS *********************************************************************************
        ...
        ElseIf txtApproved.Text = "" Then
            ...
            For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                If DT_Signatory(x)("ButtonID") = btnApprove.Tag And DT_Signatory(x)("FormID") = Tag Then
                    Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                    Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for APPROVAL of Updated Accounts Payable of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayee.Text, dDebitT.Text, cbxPreparedBy.Text)
                    Msg &= "Thank you!"
                    '******* SEND SMS *********************************************************************************
                    If DT_Signatory(x)("Phone") = "" Then
                    Else
    ...
    Private Sub CheckNotification(xCode As String, SendMail As Boolean)
        ...
        For x As Integer = 0 To DT_Signatory.Rows.Count - 1
            If DT_Signatory(x)("ButtonID") = btnCheck.Tag And DT_Signatory(x)("FormID") = Tag Then
                Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for CHECKING of Accounts Payable of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayee.Text, dDebitT.Text, cbxPreparedBy.Text)
                Msg &= "Thank you!"
                '******* SEND SMS *********************************************************************************
                If DT_Signatory(x)("Phone") = "" Then
                Else
    ...
    Private Sub ApproveNotification(xCode As String, SendMail As Boolean)
        ...
        For x As Integer = 0 To DT_Signatory.Rows.Count - 1
            If DT_Signatory(x)("ButtonID") = btnApprove.Tag And DT_Signatory(x)("FormID") = Tag Then
                Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for APPROVAL of Accounts Payable of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayee.Text, dDebitT.Text, cbxPreparedBy.Text)
                Msg &= "Thank you!"
                '******* SEND SMS *********************************************************************************
                If DT_Signatory(x)("Phone") = "" Then
                Else
    ...
    Private Sub IAcknowledge_Click(sender As Object, e As EventArgs) Handles iAcknowledge.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("AP_Status") = "FULLY PAID" Then
                ...
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("NotAP") = 1 Then
                MsgBox("Accounts Payable is not for Acknowledgement.", MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        With ACR
            ...
            Else
                ...
                Exit Sub
            End If
            Logs("Accounts Payable", "Acknowledgement", "Acknowledgement Receipt", "", "", "", "")

            .FromAccountsReceivable = True
            .FromAccountsReceivable_AP = True
            .Clear(False)
            .tabList.Visible = False
            .btnNext.Enabled = False
            .btnRefresh.Enabled = False
            ...
            .AR_Account = DataObject(String.Format("SELECT AccountCode FROM ap_details WHERE DocumentNumber = '{0}' AND Credit > 0 AND `status` = 'ACTIVE' LIMIT 1;", GridView1.GetFocusedRowCellValue("Document Number")))
            .BankID = GridView1.GetFocusedRowCellValue("BankID")
            ...
            If selectedRowHandles.Length > 1 Then
                ...
                For x As Integer = 0 To selectedRowHandles.Length - 1
                    If GridView1.GetFocusedRowCellValue("Payee") = Rows(x)("Payee") Then
                    Else
                        MsgBox("Selected Payee are not the same.", MsgBoxStyle.Information, "Info")
                        Exit Sub
                    End If
                    If Rows(x)("AP_Status") = "APPROVED" Or Rows(x)("AP_Status") = "FOR CV / ACR" Or Rows(x)("AP_Status") = "PARTIALLY PAID" Then
                    Else
    ...
    Private Sub BtnReference_Click(sender As Object, e As EventArgs) Handles btnReference.Click
        GridView1.SetFocusedRowCellValue("Reference Number", UpdateReferenceNumber("Accounts Payable", "accounts_payable", GridView1.GetFocusedRowCellValue("Reference Number"), GridView1.GetFocusedRowCellValue("ID"), GridView1.GetFocusedRowCellValue("Document Number")))
    End Sub
End Class