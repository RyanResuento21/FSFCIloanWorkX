...
Public Class FrmDueFrom
    ...
    Private Sub FrmDueFrom_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Dim DT_Status As DataTable = DataSource("SELECT 'For Checking' AS 'Status' UNION SELECT 'For Approval' UNION SELECT 'For Acknowledgement Receipt' UNION SELECT 'Partially Paid' UNION SELECT 'Fully Paid' UNION SELECT 'Non Due From' UNION SELECT 'Cancelled'")
        With cbxStatus
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        ...
        Catch ex As Exception
            TriggerBugReport("Due From - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Private Sub LblTitle_Click(sender As Object, e As EventArgs) Handles lblTitle.Click
        OpenFormHistory("Due From", lblTitle.Text)
    End Sub
    ...
    Public Sub LoadData()
        ...
        SQL &= "    DF.ID,"
        SQL &= "    DF.PayorID,"
        SQL &= "    DF.Payor AS 'Payor',"
        SQL &= "    (SELECT CONCAT('Bank ', `Code`, ' [', IF(AccountNumber = '',CheckingAccount,AccountNumber), '] ', (SELECT B.short_name FROM bank_setup B WHERE B.ID = BankID)) AS 'Bank' FROM branch_bank WHERE ID = DF.BankID) AS 'Bank', DF.BankID, "
        SQL &= "    DATE_FORMAT(DF.DocumentDate,'%M %d, %Y') AS 'Document Date',"
        SQL &= "    DF.DocumentNumber AS 'Document Number',"
        SQL &= "    DATE_FORMAT(DF.PostingDate,'%M %d, %Y') AS 'Posting Date',"
        SQL &= "    DF.ReferenceNumber AS 'Reference Number',"
        SQL &= "    DF.Explanation,"
        SQL &= "    DF.Amount AS 'Total Receivable',"
        SQL &= "    DF.Paid AS 'Total Payment',"
        SQL &= "    Employee(DF.PreparedID) AS 'Prepared By', PreparedID, CheckedID,"
        SQL &= "    BRANCH(DF.branch_id) AS 'Branch', DF.User_EmplID, DF.Branch_ID, IF(NotAR = 1,'NON DUE FROM',IF(JVNumber != '','REVERSED',IF(`Status` IN ('DELETED','CANCELLED','DISAPPROVED'),`status`,IF(AR_Status='PENDING','FOR CHECKING',IF(AR_Status='CHECKED','FOR APPROVAL',IF(AR_Status='APPROVED','FOR ACKNOLEDGEMENT RECEIPT',AR_Status)))))) AS 'AR_Status', Employee(CheckedID) AS 'Checked By', Employee(ApprovedID) AS 'Approved By', OTAC_Check, OTAC_Approve, Attach, JVNumberV2, JVNumber, NotAR, Payor_Type, "
        ...
        For x As Integer = 0 To cbxStatus.Properties.Items.Count - 1
            If cbxStatus.Properties.Items.Item(x).CheckState = CheckState.Checked Then
                ...
                If cbxStatus.Properties.Items.Item(x).Description.ToString = "Fully Paid" Then
                    FullyPaid = True
                End If
                If cbxStatus.Properties.Items.Item(x).Description.ToString = "Non Due From" Then
                    NonDueFrom = True
                End If
        ...
        Next
        If Cancelled Then
            If ForChecking = False And ForApproval = False And ForACR = False And PartiallyPaid = False And FullyPaid = False And NonDueFrom = False Then
                SQL &= " (`status` IN ('CANCELLED','DELETED','DISAPPROVED') OR IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'DISAPPROVED') "
            Else
                SQL &= " `status` IN ('ACTIVE','CANCELLED','DELETED','DISAPPROVED') AND (IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'DISAPPROVED' "
                If ForChecking Or ForApproval Or ForACR Or PartiallyPaid Or FullyPaid Or NonDueFrom Then
                    SQL &= " OR "
                End If
                If ForChecking Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'PENDING',TRUE)"
                    If ForApproval Or ForACR Or PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If ForApproval Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'CHECKED',TRUE)"
                    If ForACR Or PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If ForACR Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'APPROVED',TRUE)"
                    If PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If PartiallyPaid Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'PARTIALLY PAID',TRUE)"
                    If FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If FullyPaid Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'FULLY PAID',TRUE)"
                    If NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If NonDueFrom Then
                    SQL &= " IF(`status` = 'ACTIVE',IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'NOT DUE FROM',TRUE)"
                End If
                SQL &= ")"
            End If
        Else
            ...
            Else
                SQL &= " AND ("
                If ForChecking Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'PENDING'"
                    If ForApproval Or ForACR Or PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If ForApproval Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'CHECKED'"
                    If ForACR Or PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If ForACR Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'APPROVED'"
                    If PartiallyPaid Or FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If PartiallyPaid Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'PARTIALLY PAID'"
                    If FullyPaid Or NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If FullyPaid Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'FULLY PAID'"
                    If NonDueFrom Then
                        SQL &= " OR "
                    End If
                End If
                If NonDueFrom Then
                    SQL &= " IF(NotAR = 1,'NOT DUE FROM',AR_Status) = 'NOT DUE FROM'"
                End If
                SQL &= ")"
            End If
        End If
        If Skip_FilterLoadData Then
            SQL &= String.Format("    AND DF.DocumentNumber = '{0}'", GL_DocumentNumber)
        End If
        If cbxAll.Checked Then
        Else
            SQL &= String.Format("    AND DATE(DF.DocumentDate) BETWEEN '{0}' AND '{1}'", FormatDatePicker(dtpFrom), FormatDatePicker(dtpTo))
        End If
        If DefaultBankID > 0 Then
            SQL &= String.Format(" AND DF.BankID = '{0}'", DefaultBankID)
        End If
        SQL &= String.Format("  AND Branch_ID IN ({0}) ORDER BY DF.DocumentNumber DESC;", If(Multiple_ID = "", Branch_ID, Multiple_ID))
    ...
    Private Sub GetDocumentNumber()
        If btnSave.Text = "&Save" Then
            txtDocumentNumber.Text = DataObject(String.Format("SELECT CONCAT('DF-', '{1}', '{2}', '-', LPAD(COUNT(ID) + 1,4,'0')) FROM due_from WHERE branch_id = '{0}' AND YEAR(DocumentDate) = YEAR('{3}');", Branch_ID, Branch_Code, Format(dtpDocument.Value, "yy"), Format(dtpDocument.Value, "yyyy-MM-dd")))
        End If
    End Sub
...
#End Region
    ...
    Private Sub BtnSave_Click(sender As Object, e As EventArgs) Handles btnSave.Click
            ...
            If MsgBoxYes(mSave) = MsgBoxResult.Yes Then
                ...
                For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                    If DT_Signatory(x)("ButtonID") = btnCheck.Tag And DT_Signatory(x)("FormID") = Tag Then
                        ...
                        Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for CHECKING of Due From of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayor.Text, dDebitT.Text, cbxPreparedBy.Text)
                        Msg &= "Thank you!"
                        '******* SEND SMS *********************************************************************************
                ...
                GetDocumentNumber()
                Dim SQL As String = "INSERT INTO due_from SET"
                ...
                For x As Integer = 0 To GridView2.RowCount - 1
                    ...
                    Else
                        SQL &= "INSERT INTO df_details SET"
                        SQL &= String.Format(" DocumentNumber = '{0}', ", txtDocumentNumber.Text)
                        ...
                        ElseIf GridView2.GetRowCellValue(x, "Account").ToString.Contains("Accounts Receivable") Then
                            Dim BranchTagged As Integer = DataObject(String.Format("SELECT BranchTagged FROM account_chart WHERE `Code` = '{0}' AND `status` = 'ACTIVE';", GridView2.GetRowCellValue(x, "Account Code")))
                            Dim ARNumber As String = DataObject(String.Format("SELECT CONCAT('AR-', '{1}', '{2}', '-', LPAD(COUNT(ID) + 1,4,'0')) FROM accounts_receivable WHERE branch_id = '{0}' AND YEAR(DocumentDate) = YEAR('{3}');", Branch_ID, Branch_Code, Format(dtpDocument.Value, "yy"), Format(dtpDocument.Value, "yyyy-MM-dd")))
                ...
                For x As Integer = 0 To DT_Account.Rows.Count - 1
                    ...
                    Else
                        ...
                        SQL &= String.Format(" BusinessCode = '{0}', ", GridView2.GetRowCellValue(x, "BusinessCode"))
                        SQL &= String.Format(" DepartmentCode = '{0}', ", GridView2.GetRowCellValue(x, "Department Code"))
                        SQL &= String.Format(" PaidFor = '{0}', ", "Due From")
                        SQL &= " `Status` = 'PENDING',"
                ...
                Logs("Due From", "Save", String.Format("Add new Due From for {0} with Amount {1}.", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dAmount.Text), "", "", "", "")
                Clear()
                Cursor = Cursors.Default
        ...
        Else
            If MsgBoxYes(mUpdate) = MsgBoxResult.Yes Then
                Dim CancelledID As Integer = DataObject(String.Format("SELECT IFNULL(ID,0) FROM due_from WHERE ID = {0} AND `status` IN ('CANCELLED','DISAPPROVED')", ID))
                ...
                If txtChecked.Text = "" Then
                    ...
                    For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                        If DT_Signatory(x)("ButtonID") = btnCheck.Tag And DT_Signatory(x)("FormID") = Tag Then
                            Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                            Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for CHECKING of Updated Due From of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayor.Text, dDebitT.Text, cbxPreparedBy.Text)
                            Msg &= "Thank you!"
                            '******* SEND SMS *********************************************************************************
                            If DT_Signatory(x)("Phone") = "" Then
                            Else
                ...
                ElseIf txtApproved.Text = "" Then
                    ...
                    For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                        If DT_Signatory(x)("ButtonID") = btnApprove.Tag And DT_Signatory(x)("FormID") = Tag Then
                            Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                            Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for APPROVAL of Updated Due From of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayor.Text, dDebitT.Text, cbxPreparedBy.Text)
                            Msg &= "Thank you!"
                            '******* SEND SMS *********************************************************************************
                            If DT_Signatory(x)("Phone") = "" Then
                            Else
                ...
                Dim SQL As String = "UPDATE due_from SET"
                SQL &= String.Format(" PayorID = '{0}', ", ValidateComboBox(cbxPayor))
                SQL &= String.Format(" Payor = '{0}', ", cbxPayor.Text.InsertQuote)
                SQL &= String.Format(" Payor_Type = '{0}', ", If(cbxPayor.SelectedIndex = -1, "", drv("Type")))
                SQL &= String.Format(" PostingDate = '{0}', ", Format(dtpPostingDate.Value, "yyyy-MM-dd"))
                ...
                SQL &= String.Format("UPDATE df_details SET `status` = 'CANCELLED' WHERE DocumentNumber = '{0}' AND `status` = 'ACTIVE';", txtDocumentNumber.Text)
                Dim SQLv3 As String = ""
                ...
                Logs("Due From", "Update", Reason, Changes(), "", "", "")
                Clear()
                Cursor = Cursors.Default
    ...
    Private Function Changes()
        ...
        Try
            ...
            End If
        Catch ex As Exception
            TriggerBugReport("Due Form - Changes", ex.Message.ToString)
        End Try
    ...
    Private Sub BtnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MsgBoxYes(mDelete) = MsgBoxResult.Yes Then
            ...
            Cursor = Cursors.WaitCursor
            Dim SQL As String = String.Format("UPDATE due_from SET `status` = 'CANCELLED' WHERE ID = '{0}';", ID)
            ...
            DataObject(SQL)
            Logs("Due From", "Cancel", Reason, String.Format("Cancel Due From of {0} with Amount {1}.", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dAmount.Text), "", "", "")
            Clear()
            Cursor = Cursors.Default
    ...
    Private Sub Generate_AR(Show As Boolean, FName As String, CheckedBy As String, ApprovedBy As String)
        ...
        With Report
            .Name = String.Format("Due From of {0} - {1}", cbxPayor.Text, txtDocumentNumber.Text)
            .lblTitle.Text = "Due From"
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        ...
        If SuperTabControl1.SelectedTabIndex = 0 Then
            ...
            Logs("Due From", "Print", "[SENSITIVE] Print Due From " & txtDocumentNumber.Text, "", "", "", "")
        Else
            GridView1.OptionsPrint.UsePrintStyles = False
            StandardPrinting("DUE FROM LIST", GridControl1)
            Logs("Due From", "Print", "[SENSITIVE] Print Due From List", "", "", "", "")
        End If
        Cursor = Cursors.Default
    End Sub
    '***BUTTON CLICK
    ...
    Private Sub BtnAttach_Click(sender As Object, e As EventArgs) Handles btnAttach.Click
        ...
        With Attach
            .FolderName = "Due From-" & GridView1.GetFocusedRowCellValue("Document Number")
            .DFNumber = GridView1.GetFocusedRowCellValue("Document Number")
    ...
    Private Sub BtnCheck_Click(sender As Object, e As EventArgs) Handles btnCheck.Click
        ...
        If (User_EmplID = Empl_ID Or cbxPreparedBy.SelectedValue = Empl_ID) And Signatory = False Then
                    ...
                    With Provider
                        ...
                        If .ShowDialog = DialogResult.OK Then
                            ...
                            For x As Integer = 0 To DT_Signatory.Rows.Count - 1
                                If DT_Signatory(x)("ButtonID") = btnApprove.Tag And DT_Signatory(x)("FormID") = Tag Then
                                    Msg = String.Format("Good day," & vbCrLf, DT_Signatory(x)("Employee").ToString.Trim)
                                    Msg &= String.Format(" One Time Authorization Code is <b>{0}</b> for APPROVAL of Due From of {1} with the amount of {2} prepared by {3}." & vbCrLf, Code, cbxPayor.Text, dDebitT.Text, cbxPreparedBy.Text)
                                    Msg &= "Thank you!"
                                    '******* SEND SMS *********************************************************************************
                                    If DT_Signatory(x)("Phone") = "" Then
                                    Else
                            ...
                            DataObject(String.Format("UPDATE due_from SET `AR_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE ID = '{0}'; UPDATE due_to SET `AP_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_receivable SET `AR_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_payable SET `AP_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE loans_payable SET `AP_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}';", ID, .cbxProvider.SelectedValue, Code, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                            Logs("Due From", "Check", String.Format("Checked Due From of {0} with the total amount of {1}. [Via OTAC]", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
            ...
            End With
        Else
            If MsgBoxYes("Are you sure you check this Due From?") = MsgBoxResult.Yes Then
                Cursor = Cursors.WaitCursor
                Code = GenerateOTAC()
                Code_Approve = Code
                Dim Msg As String = ""
                ...
                DataObject(String.Format("UPDATE due_from SET `AR_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE ID = '{0}'; UPDATE due_from SET `AR_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_receivable SET `AR_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_payable SET `AP_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE loans_payable SET `AP_Status` = 'CHECKED', CheckedID = '{1}', OTAC_Approve = '{2}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}';", ID, Empl_ID, Code, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                Logs("Due From", "Check", String.Format("Checked Due From of {0} with the total amount of {1}.", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
    ...
    Private Sub BtnApprove_Click(sender As Object, e As EventArgs) Handles btnApprove.Click
        ...
        If (User_EmplID = Empl_ID Or cbxPreparedBy.SelectedValue = Empl_ID) And Signatory = False Then
                ...
                If .ShowDialog = DialogResult.OK Then
                    ...
                    With Provider
                        ...
                        If .ShowDialog = DialogResult.OK Then
                            ...
                            DataObject(String.Format(SQL & "UPDATE due_from SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE ID = '{0}'; UPDATE due_to SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE accounts_receivable SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE accounts_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}'; UPDATE loans_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{3}' WHERE JVNumberV2 = '{2}';", ID, .cbxProvider.SelectedValue, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                            Logs("Due From", "Approve", String.Format("Approved Due From of {0} with the total amount of {1}. [Via OTAC]", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
            ...
            End With
        Else
            If MsgBoxYes("Are you sure you want to approve this Due From?") = MsgBoxResult.Yes Then
                Cursor = Cursors.WaitCursor
                Dim SQL As String = String.Format("UPDATE accounting_entry SET ORDate = '{1}', `Status` = 'ACTIVE', PostStatus = 'POSTED' WHERE CVNumber = '{0}' AND `status` = 'PENDING';", txtDocumentNumber.Text, Format(dtpPostingDate.Value, "yyyy-MM-dd"))
                DataObject(String.Format(SQL & "UPDATE due_from SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE ID = '{0}'; UPDATE due_to SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_receivable SET `AR_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE accounts_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}'; UPDATE loans_payable SET `AP_Status` = 'APPROVED', ApprovedID = '{1}', PostingDate = '{4}' WHERE JVNumberV2 = '{3}';", ID, Empl_ID, Code, txtDocumentNumber.Text, FormatDatePicker(dtpPostingDate)))
                Logs("Due From", "Approve", String.Format("Approved Due From of {0} with the total amount of {1}.", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "", "")
    ...
    Private Sub BtnDisapprove_Click(sender As Object, e As EventArgs) Handles btnDisapprove.Click
        If MsgBoxYes("Are you sure you want to disapprove this Due From?") = MsgBoxResult.Yes Then
            Dim Reason As String 'Reason for change
            If FrmReason.ShowDialog = DialogResult.OK Then
                Reason = FrmReason.txtReason.Text
            Else
            ...
            Cursor = Cursors.WaitCursor
            Dim SQL As String = String.Format("UPDATE due_from SET `status` = 'DISAPPROVED' WHERE ID = '{0}';", ID)
            ...
            DataObject(SQL)
            Logs("Due From", "Delete", Reason, String.Format("Delete Due From of {0} with Amount {1}.", cbxPayor.Text.InsertQuote & " - " & txtDocumentNumber.Text, dDebitT.Text), "", "", "")
            Clear()
            Cursor = Cursors.Default
    ...
    Private Sub IAcknowledge_Click(sender As Object, e As EventArgs) Handles iAcknowledge.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AR_Status") = "FOR APPROVAL" Then
                MsgBox("Due From is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ...
            ElseIf GridView1.GetFocusedRowCellValue("NotAR") = 1 Then
                MsgBox("Due From is not for Acknowledgement.", MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        With ACR
            ...
            Else
                ...
                Exit Sub
            End If
            Logs("Due From", "Acknowledgement", "Acknowledgement Receipt", "", "", "", "")

            .FromAccountsReceivable = False
            .FromDueFrom = True
            .Clear(False)
            .tabList.Visible = False
            .btnNext.Enabled = False
            ...
            .AR_Account = DataObject(String.Format("SELECT AccountCode FROM df_details WHERE DocumentNumber = '{0}' AND Debit > 0 AND `status` = 'ACTIVE' LIMIT 1;", GridView1.GetFocusedRowCellValue("Document Number")))
            ...
            If selectedRowHandles.Length > 1 Then
                ...
                For x As Integer = 0 To selectedRowHandles.Length - 1
                    ...
                    Else
                        ...
                        Exit Sub
                    End If
                    If Rows(x)("AR_Status") = "FOR ACKNOWLEDGEMENT RECEIPT" Or Rows(x)("AR_Status") = "PARTIALLY PAID" Then
                    Else
                        MsgBox(String.Format("Selected Due From have a {0} status.", Rows(x)("AR_Status")), MsgBoxStyle.Information, "Info")
                        Exit Sub
                    End If
    ...
    Private Sub INotAR_Click(sender As Object, e As EventArgs) Handles iNotAR.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("JVNumberV2") = "" Then
                MsgBox("Due From is not auto generated.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AR_Status") = "FOR APPROVAL" Then
                MsgBox("Due From is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FULLY PAID" Or GridView1.GetFocusedRowCellValue("AR_Status") = "PARTIALLY PAID" Then
                MsgBox("Due From already have a transaction.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "CANCELLED" Or GridView1.GetFocusedRowCellValue("AR_Status") = "DELETED" Or GridView1.GetFocusedRowCellValue("AR_Status") = "DISAPPROVED" Then
                MsgBox(String.Format("Due From is already {0}.", GridView1.GetFocusedRowCellValue("AR_Status")), MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        If MsgBoxYes(String.Format("Are you sure you want to set this as {0}Due From?", If(GridView1.GetFocusedRowCellValue("NotAR") = 0, "Not ", ""))) = MsgBoxResult.Yes Then
            Dim Reason As String 'Reason for change
            If FrmReason.ShowDialog = DialogResult.OK Then
                Reason = FrmReason.txtReason.Text
            Else
            ...
            Dim SQL As String = String.Format("UPDATE due_from SET NotAR = {1} WHERE ID = '{0}';", GridView1.GetFocusedRowCellValue("ID"), If(GridView1.GetFocusedRowCellValue("NotAR") = 0, 1, 0))
            DataObject(SQL)
            Logs("Due From", "Not AR", Reason, "", "", "", "")
            LoadData()
            MsgBox("Successfully Updated!", MsgBoxStyle.Information, "Info")
        End If
    End Sub
    ...
    Private Sub BtnRename_Click(sender As Object, e As EventArgs) Handles btnRename.Click
        ...
        With Rename
            ...
            If .ShowDialog = DialogResult.OK Then
                Dim SQL As String = String.Format("UPDATE due_from SET Payor = '{1}' WHERE ID = '{0}';", GridView1.GetFocusedRowCellValue("ID"), .txtPayee.Text.InsertQuote)
                DataObject(SQL)
                Logs("Due From", "Change Payor", String.Format("Changed Payor for {0} from {1} to {2}", GridView1.GetFocusedRowCellValue("Payor").InsertQuote & " - " & GridView1.GetFocusedRowCellValue("Document Number"), GridView1.GetFocusedRowCellValue("Payor").ToString.InsertQuote, .txtPayee.Text.InsertQuote), "", "", "", "")
                LoadData()
    ...
    Private Sub ICheckRegistry_Click(sender As Object, e As EventArgs) Handles iCheckRegistry.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AR_Status") = "FOR APPROVAL" Then
                MsgBox("Due From is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf (GridView1.GetFocusedRowCellValue("AR_Status") = "REVERSED" Or GridView1.GetFocusedRowCellValue("AR_Status") = "CANCELLED" Or GridView1.GetFocusedRowCellValue("AR_Status") = "DISAPPROVED") And GridView1.GetFocusedRowCellValue("DocumentNumber_DC") = "" Then
                MsgBox(String.Format("Due To is already {0}.", GridView1.GetFocusedRowCellValue("AR_Status")), MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        With PDC
            ...
            .Payor = GridView1.GetFocusedRowCellValue("Payor")

            If GridView1.GetFocusedRowCellValue("DocumentNumber_DC") = "" Then
                .btnDelete.Enabled = False
                .DefaultBank = GridView1.GetFocusedRowCellValue("BankID")
                .dPrincipal.Value = CDbl(GridView1.GetFocusedRowCellValue("Total Receivable")) - CDbl(GridView1.GetFocusedRowCellValue("Total Payment"))
                .iTerms_C.Value = 6
                ...
                If selectedRowHandles.Length > 1 Then
                    ...
                    For x As Integer = 0 To selectedRowHandles.Length - 1
                        ...
                        Else
                            ...
                            Exit Sub
                        End If
                        TotalPayable += Rows(x)("Total Receivable")
    ...
    Private Sub BtnForRollOver_Click(sender As Object, e As EventArgs) Handles btnForRollOver.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("DocumentNumber_DC") = "" Then
                MsgBox("Due From does not have a Check Registry yet, No need for Roll Over.", MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        Dim Message As String = "Are you sure you want to set this Due From for Roll Over?"
        If GridView1.GetFocusedRowCellValue("ForRollOver") = 1 Then
            Message = "Are you sure you want to cancel the For Roll Over for this Due From "
        End If
        If MsgBoxYes(Message) = MsgBoxResult.Yes Then
            ...
            DataObject(SQL)
            Logs("Due From", "For Roll Over", Reason, "", "", "", "")
            LoadData()
            MsgBox("Successfully Updated!", MsgBoxStyle.Information, "Info")
        End If
    End Sub
    ...
    Private Sub BtnCheckManagement_Click(sender As Object, e As EventArgs) Handles btnCheckManagement.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AR_Status") = "FOR APPROVAL" Then
                ...
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("DocumentNumber_DC") = "" Then
                MsgBox("Due From does not have a Check Registry yet.", MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        Catch ex As Exception
            Exit Sub
        ...
        With FrmPDCManagement
            ...
            Logs("Due From", "Open", "PDC Management", "", "", "", "")
            Forms(FrmPDCManagement, FrmMain.PanelControl3)

            .SuperTabControl1.SelectedTabIndex = 2
            .cbxPayor.SelectedValue = GridView1.GetFocusedRowCellValue("ID")
    ...
    Private Sub IJV_Click(sender As Object, e As EventArgs) Handles iJV.Click
        Try
            ...
            ElseIf GridView1.GetFocusedRowCellValue("JVNumber") <> "" Then
                MsgBox("Due From is already reversed. Please check your data.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR CHECKING" Or GridView1.GetFocusedRowCellValue("AR_Status") = "FOR APPROVAL" Then
                MsgBox("Due From is not yet APPROVED.", MsgBoxStyle.Information, "Info")
                Exit Sub
            ElseIf GridView1.GetFocusedRowCellValue("AR_Status") = "FOR ACKNOWLEDGEMENT RECEIPT" Or GridView1.GetFocusedRowCellValue("AR_Status") = "NOT RECEIVABLE" Then
            Else
                MsgBox(String.Format("Due From is already {0}.", GridView1.GetFocusedRowCellValue("AR_Status")), MsgBoxStyle.Information, "Info")
                Exit Sub
            End If
        ...
        With JV
            ...
            Else
                ...
                Exit Sub
            End If
            Logs("Due From", "Journal Voucher", "Journal Voucher", "", "", "", "")
            .cbxLA.Enabled = False
            .cbxCV.Enabled = False
    ...
    Private Sub BtnReference_Click(sender As Object, e As EventArgs) Handles btnReference.Click
        GridView1.SetFocusedRowCellValue("Reference Number", UpdateReferenceNumber("Due From", "due_from", GridView1.GetFocusedRowCellValue("Reference Number"), GridView1.GetFocusedRowCellValue("ID"), GridView1.GetFocusedRowCellValue("Document Number")))
    End Sub
End Class