...
Public Class FrmCaseViewingD

    Private Sub FrmCaseViewing_D_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        ...
        Catch ex As Exception
            TriggerBugReport("Case Viewing D - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Private Sub LoadData()
        ...
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'DISMISSED WITH PREJUDICE' THEN Amount END),0) AS 'A_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'DISMISSED WITH PREJUDICE' THEN Account END),0) AS 'A_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'DISMISSED WITHOUT PREJUDICE' THEN Amount END),0) AS 'B_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'DISMISSED WITHOUT PREJUDICE' THEN Account END),0) AS 'B_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR REFILING' THEN Amount END),0) AS 'C_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR REFILING' THEN Account END),0) AS 'C_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory IN ('DISMISSED WITH PREJUDICE','DISMISSED WITHOUT PREJUDICE','FOR REFILING') THEN Amount END),0) AS 'T_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory IN ('DISMISSED WITH PREJUDICE','DISMISSED WITHOUT PREJUDICE','FOR REFILING') THEN Account END),0) AS 'T_Account'"
        SQL &= " FROM branch B LEFT JOIN (SELECT COUNT(M.ID) AS 'Account', SUM(Ledger_Balance(M.AccountNumber,M.MortgageID)) AS 'Amount', M.BranchID, SubCategory(SubCategoryID) AS 'SubCategory', SubCategoryID, CategoryID FROM case_main M WHERE M.`status` = 'ACTIVE' AND CategoryID = 9 GROUP BY SubCategoryID, BranchID) C"
        ...
        Next
        DT.Rows.Add(1000, "", "               T O T A L", "", A_Amount, A_Account, B_Amount, B_Account, C_Amount, C_Account, T_Amount, T_Account)
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Cursor = Cursors.WaitCursor
        Dim Report As New RptCaseMappingD
        With Report
            ...
            Logs("Case Mapping", "Print", "[SENSITIVE] Print Case Mapping Dismissed", "", "", "", "")
            .ShowPreview()
        End With
        Cursor = Cursors.Default
    End Sub
    ...
    Private Sub PieChart(Chart As ChartControl, Type As String)
        ...
        With Product
            .Columns.Add("Caption")
            .Columns.Add("Value")
            .Rows.Add("Dismissed with Prejudice", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "A_" & Type))
            .Rows.Add("Dismissed without Prejudice", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "B_" & Type))
            .Rows.Add("For Refiling", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "C_" & Type))
        End With
        ' Create a pie chart
        Chart.Series.Clear()
        Try
            ...
            Chart.Titles(0).Text = Type & " Percentage [Dismissed]"
        Catch ex As Exception
            Cursor = Cursors.Default
        End Try
        Cursor = Cursors.Default
    ...
    Private Sub LineChart(Chart As ChartControl)
        ...
        With Chart
            ...
            For x As Integer = 0 To BandedGridView1.RowCount - 2
                ...
                .Series.Add(Series1)
                .Series.Add(Series2)
                .Titles(0).Text = "Dismissed Amount | Account"
            Next
        End With
        Cursor = Cursors.Default
    End Sub
    ...
    Private Sub BandedGridView1_DoubleClick(sender As Object, e As EventArgs) Handles BandedGridView1.DoubleClick
        ...
        With View
            .lblTitle.Text = "CASE VIEWING DISMISSED LIST"
            ...
            End With
            Dim SQL As String
            Dim DT_Archived As DataTable = DataSource("SELECT ID, SubCategory FROM case_subcategory WHERE `status` = 'ACTIVE' AND CategoryID = 9 ORDER BY `Rank`;")
            For x As Integer = 0 To DT_Archived.Rows.Count - 1
                ...
                SQL &= String.Format(" FROM case_main M WHERE `status` = 'ACTIVE' AND CategoryID = 9  AND SubCategoryID = {0} ORDER BY `Defendant`;", DT_Archived(x)("ID"))
