...
Public Class FrmCaseViewingA

    Private Sub FrmCaseViewing_A_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub FixUI(ApplyUI As Boolean)
        ...
        Catch ex As Exception
            TriggerBugReport("Case Viewing A - FixUI", ex.Message.ToString)
        End Try
    End Sub
    ...
    Private Sub LoadData()
        ...
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'NEWLY FILED' THEN Amount END),0) AS 'A_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'NEWLY FILED' THEN Account END),0) AS 'A_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR RAFFLE' THEN Amount END),0) AS 'B_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR RAFFLE' THEN Account END),0) AS 'B_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR DELIVERY OF SUMMONS' THEN Amount END),0) AS 'C_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR DELIVERY OF SUMMONS' THEN Account END),0) AS 'C_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR PMC' THEN Amount END),0) AS 'D_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR PMC' THEN Account END),0) AS 'D_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR JDR' THEN Amount END),0) AS 'E_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'FOR JDR' THEN Account END),0) AS 'E_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'PRE-TRIAL' THEN Amount END),0) AS 'F_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'PRE-TRIAL' THEN Account END),0) AS 'F_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'TRIAL' THEN Amount END),0) AS 'G_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'TRIAL' THEN Account END),0) AS 'G_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'SUBMITTED FOR DECISION' THEN Amount END),0) AS 'H_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory = 'SUBMITTED FOR DECISION' THEN Account END),0) AS 'H_Account',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory IN ('NEWLY FILED','FOR RAFFLE','FOR DELIVERY OF SUMMONS','FOR PMC','FOR JDR','PRE-TRIAL','TRIAL','SUBMITTED FOR DECISION') THEN Amount END),0) AS 'T_Amount',"
        SQL &= "   IFNULL(SUM(CASE WHEN C.SubCategory IN ('NEWLY FILED','FOR RAFFLE','FOR DELIVERY OF SUMMONS','FOR PMC','FOR JDR','PRE-TRIAL','TRIAL','SUBMITTED FOR DECISION') THEN Account END),0) AS 'T_Account'"
        SQL &= " FROM branch B LEFT JOIN (SELECT COUNT(M.ID) AS 'Account', SUM(Ledger_Balance(M.AccountNumber,M.MortgageID)) AS 'Amount', M.BranchID, SubCategory(SubCategoryID) AS 'SubCategory', SubCategoryID, CategoryID FROM case_main M WHERE M.`status` = 'ACTIVE' AND CategoryID = 1 GROUP BY SubCategoryID) C"
        ...
        Next
        DT.Rows.Add(1000, "", "               T O T A L", "", A_Amount, A_Account, B_Amount, B_Account, C_Amount, C_Account, D_Amount, D_Account, E_Amount, E_Account, F_Amount, F_Account, G_Amount, G_Account, H_Amount, H_Account, T_Amount, T_Account)
    ...
    Private Sub BtnPrint_Click(sender As Object, e As EventArgs) Handles btnPrint.Click
        Cursor = Cursors.WaitCursor
        Dim Report As New RptCaseMappingA
        With Report
            ...
            Logs("Case Mapping", "Print", "[SENSITIVE] Print Case Mapping Ongoing", "", "", "", "")
            .ShowPreview()
        End With
        Cursor = Cursors.Default
    End Sub
    ...
    Private Sub PieChart(Chart As ChartControl, Type As String)
        ...
        With Product
            .Columns.Add("Caption")
            .Columns.Add("Value")
            .Rows.Add("Newly Filed", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "A_" & Type))
            .Rows.Add("For Raffle", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "B_" & Type))
            .Rows.Add("For Delivery of Summons", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "C_" & Type))
            .Rows.Add("For PMC", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "D_" & Type))
            .Rows.Add("For JDR", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "E_" & Type))
            .Rows.Add("Pre-Trial", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "F_" & Type))
            .Rows.Add("Trial", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "G_" & Type))
            .Rows.Add("Submitted for Decision", BandedGridView1.GetRowCellValue(BandedGridView1.RowCount - 1, "H_" & Type))
        End With
        ' Create a pie chart
        Chart.Series.Clear()
        Try
            ...
            Chart.Titles(0).Text = Type & " Percentage [Ongoing]"
        Catch ex As Exception
            Cursor = Cursors.Default
        End Try
        Cursor = Cursors.Default
    ...
    Private Sub LineChart(Chart As ChartControl)
        ...
        For x As Integer = 0 To BandedGridView1.RowCount - 2
            ...
            Chart.Titles(0).Text = "Ongoing Amount | Account"
        Next
        Cursor = Cursors.Default
    End Sub
    ...
    Private Sub BandedGridView1_DoubleClick(sender As Object, e As EventArgs) Handles BandedGridView1.DoubleClick
        ...
        With View
            .lblTitle.Text = "CASE VIEWING ONGOING LIST"
            ...
            End With
            Dim SQL As String
            Dim DT_Archived As DataTable = DataSource("SELECT ID, SubCategory FROM case_subcategory WHERE `status` = 'ACTIVE' AND CategoryID = 1 ORDER BY `Rank`;")
            For x As Integer = 0 To DT_Archived.Rows.Count - 1
                ...
                SQL &= String.Format(" FROM case_main M WHERE `status` = 'ACTIVE' AND CategoryID = 1  AND SubCategoryID = {0} ORDER BY `Defendant`;", DT_Archived(x)("ID"))
